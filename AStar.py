...
class State:
    ...
    def getEclud(self):
        x = (self.max_x - 1 - self.own_piece.get_x()) ** 2
        y = (self.max_y - 1 - self.own_piece.get_y()) ** 2
        dis = (x + y) ** 0.5
        return dis

    def heuristic_function(self):
        return self.get_action_cost() + self.getEclud()

    def __lt__(self, other):
        return self.heuristic_function() < other.heuristic_function()
...
def search(rows, cols, grid, enemy_pieces, own_pieces, goals):
    ...
    path_cost = [[-1 for i in range(cols)] for j in range(rows)]
    pq = [start_state]
    ...
    while pq:
        ...
        if not visited[piece_y][piece_x] or (
                path_cost != -1 and curr_state.get_action_cost() < path_cost[piece_y][piece_x]):
            visited[piece_y][piece_x] = True
            path_cost[piece_y][piece_x] = curr_state.get_action_cost()
